...
// Sample schema-based table data
const schemaObjects = [
  { schema: "dbo", name: "Table 1" },
  { schema: "dbo", name: "Table 2" },
  { schema: "sales", name: "Sales Report" },
  { schema: "sales", name: "Customer Orders" },
  { schema: "hr", name: "Employee Records" },
  { schema: "hr", name: "Payroll Data" },
];
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", flexDirection: "row", height: "100vh", padding: "20px" }}>
        {/* Dropdown Section */}
        <div style={{ flexBasis: "30%", display: "flex", flexDirection: "column", gap: "20px", padding: "10px" }}>
          <Autocomplete
            options={schemaObjects}
            getOptionLabel={(option) => `${option.schema}.${option.name}`}
            groupBy={(option) => option.schema} // Grouping by schema
            value={selectedObject}
            onChange={(event, newValue) => setSelectedObject(newValue)}
            renderInput={(params) => <TextField {...params} label="Select Object" variant="outlined" />}
          />
        ...
        >
          ...
          >
            {selectedObject
              ? `CREATE TABLE [${selectedObject.schema}].[${selectedObject.name}] (
  [ID] INT PRIMARY KEY,
  [Name] NVARCHAR(100),
  [CreatedAt] DATETIME DEFAULT GETDATE()
);`
              : "Select an object to preview the DDL"}
          </pre>
        </div>
      </div>
      ...
      >
        ...
        <a href="./AuthenticationPage" style={buttonStyle}>
          CONFIRM
        </a>
        <Button style={buttonStyle} onClick={handleUpdate} disabled={!selectedObject}>
          UPDATE
        </Button>
      </div>
    </>
